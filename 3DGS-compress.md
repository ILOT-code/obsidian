...
在对所有的高斯基元计算了其对应的球形区域包含的高斯基元数量后（不妨称为该球形区域的度），作者进一步定义了高斯基元的冗余度：与该高斯基元相交的所有球形区域中，最小的度。这依然是一个保守的计算方法。设某高斯基元和 $a,b$ 两个球形区域相交，$a$ 的度很大（为 10），$b$ 则很小（为 1），如果以 10 作为其冗余度，h 这个高斯元
