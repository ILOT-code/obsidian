...
和质心的距离被重写为：
$$
\mathcal{D}(\mathbf{x},\mathbf{c})=S(\mathbf{x})\| \mathbf{x} - \mathbf{c}\|_{2}^{2}
$$

在对某一向量参数进行聚类的 h 过程中，那些敏感度超出阈值的参数不会进行计算，而是作为一个条目直接加入聚类完成的码本中。这些参数的量化可能会带来严重的后果。
...
$$
j=\arg\underset{m}{\min} - \frac{\log p_{m}}{\lambda^{s}}+(\mathbf{s}_{i}-CB^{s}[m])^{2}
$$
训练完成后，该概率模型被 AC 用来编解码。
