...
我们看$\int_0^{\xi}{r_{j}^{\prime}\left( \mathbf{x},\mu \right)}\mathrm{d}\mu$这一部分，这条光线段可能会错过或与$j$这个重建核的支撑域相交。前者不必考虑，后者就有两种情况，一种是完全穿过，一种是光线段的一个顶点在该支撑域内部。作者进一步引入了一个“ignore self-occlusion”的假设。这个假设说的是不那么细致的考虑每个基元内部的不透明度（opacity）的变化，直接将穿透整个基元后所消耗的那么多为最小单位。所以公式上就是将连乘符号内部的那个积分的积分上限调整成$L$，那样的话，整个连乘符号将跟外面的那个积分无关，于是就可以拿出来了：
$$\begin{align}
