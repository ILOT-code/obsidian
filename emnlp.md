...
我们注意到该算法存在很强的局部性质，点的 $\mathbf{Score}$ 是点的自身权重与 $\mathbf{Drop}$ 中所有点的边权之和，这意味着，在算法进行的早期阶段，新选择的点很大程度上依赖于之前选择的点，倘若初始选择的点对于全局而言不是特别好，会影响后面点的选择。因此，我们翻转了算法搜索的顺序，形成了算法二。
