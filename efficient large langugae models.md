...
例如，Micikevicius 等人（2018）提出了自动混合精度（AMP），以保持全精度（FP 32）权重的副本用于更新，而权重、激活和梯度则以 FP 16 存储用于算术运算。值得注意的是，改进版的 AMP（Facebook AI Research (FAIR), 2023）已取消了 FP 32 权重的副本，但优化器（AdamW）内部仍使用 FP 32。同时，Rae 等人（2022）证明，AMP 中的 FP 16 由于数值范围受限会导致精度损失。为了解决这一问题，提出了具有更大动态范围（即指数位数更多）的 Brain Floating Point（BFLOAT 16）（Kalamkar 等人，2019；Burgess 等人，2019），以实现更好的训练性能。