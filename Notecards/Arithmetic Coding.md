...
一下证明基于一个基本假设：如果 $U=0$，那么 $L$ 和 $L'$，$R$ 和 $R'$ 那些正在处理的位是相等的，已经处理完毕的那些位 (前面那些)被存储起来，和 $L,R$ 的那些位完全一致。在最开始的时候，算法还未处理任何符号，这显然成立，在第一步基于此假设，又结合下面的证明如果，可以得到任意情况这假设都成立。而对于 $U！=0$ 的情况，在下面的证明中可以看到假设这四个数的
分为两种情况，$U=0$ 和 $U!=0$.
