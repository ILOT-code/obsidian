...
假设，在 $A_{n+1}$ 结束之后，$U=0$，也就是最后的真正的 $L$ 是 0 开头，真正的 $R$ 是 1 开头，我们的 $V$ 需要取它们中间的数，那就可以把 $V$ 直接补一个 1，加无穷个 0.
假设，在 $A_{n+1}$ 结束之后，$U！=0$，也就是最后的真正的 $L$ 是 0 开头，加 $U$ 个 1，，$R$ 是 1 开头，加 $U$ 个 0，我们的 $V$ 需要取它们中间的数，那还是可以就可以把 $V$ 直接补一个 1，加无穷个 0.