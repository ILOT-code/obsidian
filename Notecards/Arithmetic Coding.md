...
我要证明这样的结论：任意时刻进入到该函数时，设真正的区间（无穷的）是 $L，R$，此刻我们算法内部的有限区间 (也就是 `low`, `high`)是 $L'$, $R'$，还有一个 $numUnderflow$ （简写为 $U$）。该函数进行编码（编码新的元素），对 $L'$, $R'$ ，$U$ 进行任何一次操作（一次这个函数内部可能有很多次操作），是完全等价于对 $L,R$ 作基本的（无移位）算术编码的操作。
