...
一些工作侧重于对这些网络进行结构上的更改。DNeRV[^23]采用杂交式的方案，不同的是，latent 由两部分组成，除HNeRV 固有的那部分之外，还从相邻帧之间的残差生成一个额外的嵌入。HiNeRV[^26]利用卷积层和插值层搭建了网络架构，它是一种patch-wise的模式，并使用 patch 的坐标在一个很小的local grid中索引出特征，输入到中间层去。
