...
$L$$I(\cdot)$指的是光强。然后$f_{c}^{\prime}\left( \mathbf{x},\xi \right)$是“消光系数”，用于建模光的“自我遮挡”，它是一个介于$[0,1]$的数。然后也有熟悉的负指数作为衰减因子，$c\left( \mathbf{x},\xi \right)$是“发射系数”。它的意思就是，空间中的每一个点，都是一个发光点，有自己的发射系数还有自身对光的吸收能力，对自身光的吸收能力与对其它光的阻挡能力，是负相关的，假设它能完全吸收自己的光，那就是一个透明点了，完全不会阻挡其它光。
