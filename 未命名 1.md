...
1. 把$r_{k}^{\prime}\left( \mathbf{x},\xi \right)$显著不为0的那部分定义域称为“支撑域”。只有支撑域上的空间才是我们应当关心的，非支撑域在建模过程中可以忽略掉。
2. 一条光路上的采样点，这些支撑集都是不重叠的。

于是我们自然的假设，对于每个$k$所对应的$r_{k}^{\prime}\left( \mathbf{x},\xi \right)$，在其支撑域内，$c\left( \mathbf{x},\xi \right)$近似为一个常数。这样我们可以将$c\left( \mathbf{x},\xi \right)$从积分里拿出来。同时再对指数函数进行泰勒展开，$e^{-x}\approx1-x$，所以此时可以推出：
