...
**量化生成式的中形语言模型**
生成式的模型的量化方法很稀少，区别在于量化误差在逐个 token 的生成过程中累积，因此量化生成语言模型通常是一个更复杂的问题。
...
FlashAttention 把重点放在减少内存访问开销上，实现训练和推理的加速。它并不是近似的方法，结果是完全一样的。
![[Pasted image 20240409174419.png|400]]
...
## DYNAMIC NETWORKS
扩大模型规模能够提高性能，但也会带来计算成本、内存需求的上升。动态神经网络仅使用网络的一个子集来处理输入。最广泛使用的就是 MOE 模型。
...
#### Routing Method
路由网络面临的重要问题是负载不均衡。由于门控网络的稀疏结构，有可能大部分 token被分配到一个专家中，而其他专家却很少，造成资源的浪费。

GShard 提出了 3 种方法：1，限定每个专家可以被分配的任务数。2，额外的损失函数，给与不平衡的分配额外的损失。3，随机路由。

除此之外，还有一些基于学习的方法：Expert choice 并不是让每个 token 选择特定的专家，而是让每个专家去选择特定的 token，实现完美的负载均衡。Clark 则是使用强化学习的方法来做。还有一些无学习的方法：Hash layer 使用预先设定好的 token 到 exoerts 的哈希函数来完成这样的分配。

稀疏性也带来了一些问题：降低了模型的容量和收敛速度。DeepSeek-MoE 使用一组固定的专家来参与所有的任务，而其他一些专家则用来被选择。DSselect-k 则解决了稀疏路由的不连续的问题，加快了收敛速度。
...
MoE 被视为使用内存换区模型容量的一种技术，可以使用其他模型压缩的手段来压缩，以减少内存消耗。

#### Model Compression

Z-code 在验证集上，根据专家被选中的比率，去除掉一些不重要的专家。Chen 在微调阶段逐步去除一些专家，只留下对下游任务而言最专业的那个专家。MPOE 把专家网络的权重分解成中心参数与辅助参数，中心参数可以在专家中共享，辅助参数用来捕获一些特有的特征。
