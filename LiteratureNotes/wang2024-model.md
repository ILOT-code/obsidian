...
最佳脑损伤（OBD）[114]和最佳脑外科医生（OBS）[115]代表了早期作品中的二阶剪枝方法。两者都利用损失函数的 Hessian 矩阵来选择性地消除特定参数，同时最大限度地减少对损失的影响。为了简化计算，两种方法都在一定程度上简化了 Hessian 矩阵的计算。然而，OBD 仅计算 Hessian 矩阵的对角项，而 OBS 还考虑非对角项的影响。这些方法为许多后续方法提供了灵感。 Optimal BERT Surgeon [116] 将 OBS 的原理扩展到 BERT 的背景下，与一些基于幅度的剪枝方法 [98]、[104]、[106] 和一阶剪枝方法 [108] 相比，产生了更好的结果。，[111]。