...
在时间步 $t$ 时，从先前 buffer 中提取出 $\hat{B}_{t-1}$，并固定其参数，训练残余量 $R_{t}$ 以及 $C_{t}$. 存储 $C_{t}$ 以及 $R_{t}$
引入额外的正则项 $\mathcal{L}_{reg}=||R_{t}||_{1}$来保证时间上的连续性。
>[!Qus]
> $R$ 和 $B$ 的大小是一样的，那这样训练除了确实引入了帧间参考外，存储的数据量不是没太大变化？

![[Pasted image 20240905162319.png]]

### 端到端
使用拉普拉斯分布为 $C_{t},R_{t}$ 的概率建模，使用加噪声的方式模拟量化。
$$
